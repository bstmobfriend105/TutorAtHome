...
@property (nonatomic, strong) TKAWSCTListView *awsListView;
...
- (void)addNotification {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(loadAWSCourseware)
                                                 name:sLoadAWSCourseware
                                               object:nil];
...
- (void)loadAWSCourseware {
    if (_miniWB) {
        [_miniWB downloadCoursewareFromAWS];
...
- (TKMiniWhiteBoardView *)miniWB {
    if (!_miniWB) {
        ...
        if (_iParamDic != nil) {
            _miniWB.loginuserid = [_iParamDic objectForKey:@"loginuserid"];
        }
        [_miniWB createRoomDirectory];
        [_miniWB switchStates:TKMiniWhiteBoardStatePrepareing];
        ...
        tk_weakify(self);
        _miniWB.coursewareButtonClickBlock = ^(UIButton *sender, NSArray *dataArray) {
            if (sender.selected) {
                if (!weakSelf.awsListView) {
                    ...
                    weakSelf.awsListView =
                        [[TKAWSCTListView alloc] initWithFrame:CGRectMake(x, y, showWidth, showHeight)
                                                   andTitle:@"dd"
                                                       from:nil];
                    weakSelf.awsListView.courseArray = dataArray;
                    [weakSelf.awsListView show:weakSelf.view];
                    weakSelf.awsListView.dismissBlock = ^{

                        weakSelf.miniWB.cloudBtn.selected = NO;
                        weakSelf.awsListView              = nil;
                    };
                }
            } else {
                [weakSelf tapOnViewToHide];
...
- (void)tapOnViewToHide {
    ...
    if (self.awsListView) {
        [self.view addSubview:self.awsListView];
        self.miniWB.cloudBtn.selected = YES;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5f * NSEC_PER_SEC)),
                       dispatch_get_main_queue(),
                       ^{ self.miniWB.cloudBtn.selected = NO; });
        [self.awsListView hidden];
        self.awsListView = nil;
    }
